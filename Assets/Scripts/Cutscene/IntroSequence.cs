...
public class IntroSequence : MonoBehaviour
{
    ...
    // --- NIEUWE FUNCTIE: ONDERTITELING ---
    IEnumerator PlayWakeUpDialogue()
    {
        // We wachten eerst 1.5s, net als het geluid in PlaySoundDelayed
        yield return new WaitForSeconds(1.5f);

        // "no no no N-Not here..."
        SubtitleManager.Instance.ShowSubtitle("no no no N-Not here...", 2.5f);
        yield return new WaitForSeconds(2.5f); // 2.5 tekst + 0.5 break

        // "Please..."
        SubtitleManager.Instance.ShowSubtitle("Please...", 1.0f);
        yield return new WaitForSeconds(1.0f); // 1.5 tekst + 0.3 break

        // "not here."
        SubtitleManager.Instance.ShowSubtitle("not here.", 1.0f);
        yield return new WaitForSeconds(1.0f); // 2.0 tekst + 0.4 break

        // "I need t--to..."
        SubtitleManager.Instance.ShowSubtitle("I need t--to...", 1.5f);
        yield return new WaitForSeconds(1.0f); // 2.0 tekst + 0.3 break

        // "find the files."
        SubtitleManager.Instance.ShowSubtitle("find the files.", 3.0f);
    }
}